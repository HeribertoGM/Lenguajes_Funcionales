...
find ch lst = (map (\(id, _) -> id) . filter (\(_, str) -> head str == head ch)) lst
...
append [] [] = []
append (h1:t1) (h2:t2) = (h1++h2): append t1 t2
...
findEvenVertices (Graph []) = []
findEvenVertices (Graph ((x: xs): t)) = if mod (length xs) 2 == 0
    then x: findEvenVertices (Graph t)
    else findEvenVertices (Graph t)
