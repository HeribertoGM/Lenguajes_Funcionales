...
helper(N, Lst, I) -> if 
	N == hd(Lst) -> I;
	true -> helper(N, tl(Lst), I+1)
	end.

index(N, Lst) -> helper(N, Lst, 0).

% === firstn ===

firstn(Lst, N) -> if
	N > 0 -> [hd(Lst) | firstn(tl(Lst), (N-1))];
	true -> []
	end.
...
lastn(Lst, N) -> if
	N > 0 -> lastn(tl(Lst), (N-1));
	length(Lst) == 0 -> [];
	true -> [hd(Lst) | lastn(tl(Lst), (N-1))]
	end.

% === sum ===

sum([]) -> 0;
sum(Lst) when is_list(hd(Lst)) -> sum(hd(Lst)) + sum(tl(Lst));
sum(Lst) -> hd(Lst) + sum(tl(Lst)).

% === even ===

even([]) -> [];
even(Lst) when (hd(Lst) rem 2) == 0 -> [hd(Lst) | even(tl(Lst))];
even(Lst) -> even(tl(Lst)).
...
maskedSum([], []) -> 0;
maskedSum([H1|T1], [H2|T2]) when H2 -> H1 + maskedSum(T1, T2);
maskedSum([_|T1], [_|T2]) -> maskedSum(T1, T2).

% === evaluate ===

evaluate([], _) -> 0;
evaluate([H|T], X) -> (H * (math:pow(X, length(T)))) + evaluate(T, X).
