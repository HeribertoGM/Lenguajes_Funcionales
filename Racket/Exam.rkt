...
(define (takeAux lst n)
  (if (= n 0)
      '()
      (cons (car lst) (takeAux (cdr lst) (- n 1)))
  )
)

(define (exam.takeif lst n f)
  (if (> (length (myFilter f lst)) n)
      (takeAux (myFilter f lst) n)
      (myFilter f lst)
  )
)
...
(define (exam.leaves tree)
  (cond
    [(null? tree) '()]
    [(list? (car tree)) (append (exam.leaves (car tree)) (exam.leaves (cdr tree)))]
    [else (if (isLeave tree) (cons (car tree) (exam.leaves (cdr tree))) (exam.leaves (cdr tree)))]
  )
)
...
(define (getIndex lst i n)
  (if (null? lst)
      '()
      (if (equal? (car lst) i)
          n
          (getIndex (cdr lst) i (+ n 1))
      )
  )
)

(define (exam.sdiag m)
  (map (lambda (row) (getItem row (- (length m) (getIndex m row 0) 1))) m)
)
...
(define (exam.select table cols)
  (let ((indexs (map (lambda (i) (index-of (car table) i)) cols)))
    (map (lambda (row) (filter (lambda (i) (list? (member (index-of row i) indexs))) row)) table)
  )
)
