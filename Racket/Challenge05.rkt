...
(define (get-balance movements)
  (apply +
         (map (lambda (m) (if (equal? (first m) "income") (last m) (* -1 (last m)))) movements)
  )
)
...
(define (replaceby0 row n)
  (if (equal? n 0)
      row
      (cons 0 (replaceby0 (cdr row) (- n 1)))
  )
)
...
(define (toUpperTriangularAux matrix i)
  (if (null? matrix)
      '()
      (cons (replaceby0 (car matrix) i) (toUpperTriangularAux (cdr matrix) (+ i 1)))
  )
)

(define (toUpperTriangular matrix)
  ;;(toUpperTriangularAux matrix 0)
  (map (lambda (row) (replaceby0 row (index-of matrix row))) matrix)
)
